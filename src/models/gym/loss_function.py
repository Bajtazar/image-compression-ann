...
class NormalDistributionRateDistortionLoss(Module):
    ...
    @staticmethod
    def __cdf(mean_: Tensor, standard_deviation: Tensor, points: Tensor) -> Tensor:
    ...
    def __latent_rate(self, mean_: Tensor, stddev: Tensor, latent: Tensor) -> Tensor:
        upper_bound = self.__cdf(mean_, stddev, latent + 0.5)
        lower_bound = self.__cdf(mean_, stddev, latent - 0.5)
